...
void trimEnd(char *str) { // Tira o \n que o fgets lê junto com a variavel pra
...
int jogo(void) {
  ...
  while (chances > 0) {
    ...
    // Se o usuário digitou algo que não é letra, apenas insiste sem queimar uma
    // chance.
    if (!ehLetra(tentativa))
      continue;

    // Se o usuário digitou algo que ele já tentou antes (seja errando ou
    // acertando), apenas insiste sem queimar uma chance.
    int jaTentou = 0;
    for (i = 0; erros[i] != 0; i++) {
